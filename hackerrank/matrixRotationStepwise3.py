...
os.chdir('F:\\code\\python\\hackerrank')

with open('sampleRotation1.txt') as file:
    ip = file.readline().split();
    numrows = int(ip[0])
    numcols = int(ip[1])
    rotate = int(ip[2])
    arr = [[int(col) for col in row] for row in (file.readline().split() for i in range(numrows))]
